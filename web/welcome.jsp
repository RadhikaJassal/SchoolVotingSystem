...
<%
    ...
    try{
                    ...
                    String strSQL= new String("SELECT * FROM ROOT.STUDENTS WHERE REGID=? and SESSION=?");
            ...
            <div class="rows">
                <div class="col-sm-8">
                    ...
                    <form action="vote" method="POST">
                        <select name="toVote">
                            <%
                                pstmt = conn.prepareStatement("SELECT REGID,FIRSTNAME from ROOT.STUDENTS WHERE VOTED=TRUE");
                                a = pstmt.executeQuery();
                                while(a.next()){
                     ...
                     <table border="2px">
                         ...
                         <%
                             ...
                             a = stmt.executeQuery("SELECT VOTEDFOR,count(VOTEDFOR) from ROOT.STUDENTS GROUP BY VOTEDFOR order by count(VOTEDFOR) desc");
                             while(a.next()){
                                 if(a.getLong(1)==0)continue;
                                 //pstmt = conn.prepareStatement("SELECT FIRSTNAME FROM ROOT.STUDENTS WHERE REGID=?");
